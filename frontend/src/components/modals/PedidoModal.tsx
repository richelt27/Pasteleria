...
const PedidoModal = ({ isOpen, onClose, onSave }: PedidoModalProps) => {
    ...
    const [cart, setCart] = useState<{ id_producto: number, nombre: string, cantidad: number, precio: number }[]>([]);

    // Form fields
    const [fechaEntrega, setFechaEntrega] = useState('');
    const [tipoEntrega, setTipoEntrega] = useState('RECOJO_TIENDA'); // DELIVEY
    const [direccion, setDireccion] = useState('');
    const [costoDelivery, setCostoDelivery] = useState(10); // Nuevo state para precio variable
    ...
    const [drivers, setDrivers] = useState<{ id_usuario: number, nombre_completo: string }[]>([]);
    ...
    const fetchDrivers = async () => {
        // Fetch users and filter by role 'Delivery'
        try {
            ...
            if (res.ok) {
                ...
                setDrivers(users.filter(u => u.rol?.nombre === 'Delivery' && u.estado === 'ACTIVO'));
            }
        } catch (error) {
            console.error(error);
        }
